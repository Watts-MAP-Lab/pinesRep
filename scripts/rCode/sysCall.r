...
data_jags <- list(
  N = nrow(tmp.dat),
  x = round(scale(tmp.dat[,all.mods$Var2[rowID]])[,] + abs(min(scale(tmp.dat[,all.mods$Var2[rowID]])[,])),0),
  y = round(scale(tmp.dat[,all.mods$Var1[rowID]])[,] + abs(min(scale(tmp.dat[,all.mods$Var1[rowID]])[,])),0),
...
file.out <- paste("./data/brmsModsOut/model_scaleXY_NB_allmods_", rowID, ".RDS", sep='')
